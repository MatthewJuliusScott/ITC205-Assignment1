		...
		EventQueue.invokeLater(() -> {
			try {
				...
				Carpark carpark = new Carpark("Bathurst Chase", 3,
				        adhocTicketDAO, seasonTicketDAO,
				        Carpark.TARRIF_SHORT_STAY);

				ISeasonTicket t1 = new SeasonTicket("S1111", "Bathurst Chase",
				        0L, 0L);
				...
				@SuppressWarnings("unused")
				PaystationController payController = new PaystationController(
				        carpark, pui);

				@SuppressWarnings("unused")
				ExitController exitController = new ExitController(carpark,
				        xgate, xis, xos, xui);
